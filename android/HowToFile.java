...
/*
내부 저장소에 파일 저장
내부 저장소에 파일을 저장할 경우, 다음 두 메서드 중 하나를 호출하여 적합한 디렉터리를 File로 얻을 수 있습니다.
...
이러한 디렉터리 중 하나에서 새 파일을 생성하려면 File() 생성자를 사용하고 내부 저장소 디렉터리를 지정하는 상기 메서드 중 하나를 통해 제공되는 File을 전달하면 됩니다. 예:
*/
...
예를 들어 다음 메서드는 URL에서 파일 이름을 추출한 후 해당 이름을 사용하여 앱의 내부 캐시 디렉터리에 파일을 생성합니다.
*/
...
/*
참고: 앱의 내부 저장소 디렉터리는 Android 파일 시스템의 특별한 위치에 있는 앱의 패키지 이름으로 지정됩니다. 엄밀히 말하면, 파일 모드를 읽기 가능으로 설정할 경우 다른 앱이 내부 파일을 읽을 수 있습니다. 하지만 이는 다른 앱도 여러분 자신의 앱 패키지 이름 및 파일 이름을 알아야 가능합니다. 다른 앱은 여러분 자신의 내부 디렉터리를 탐색할 수 없으며 명시적으로 읽기 가능 및 쓰기 가능으로 파일을 설정하지 않으면 파일을 읽거나 쓸 수 없습니다. 따라서 MODE_PRIVATE을 내부 저장소 내 파일에 사용하는 한, 다른 앱이 이러한 파일에 액세스할 수 없습니다
*/
